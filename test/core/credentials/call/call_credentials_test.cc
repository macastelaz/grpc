...
TEST_F(CredentialsTest, TestComputeEngineCredsWithFullRegionalAccessBoundary) {
  SetEnv("GOOGLE_AUTH_REGIONAL_ACCESS_BOUNDARY_ENABLED", "true");
  ExecCtx exec_ctx;
  std::string emd =
      "authorization: Bearer ya29.AHES6ZRN3-HlhAPya30GnW_bHSb_, "
      "x-allowed-locations: us-west1, x-allowed-locations: us-east1";
  const char expected_creds_debug_string[] =
      "GoogleComputeEngineTokenFetcherCredentials{"
      "OAuth2TokenFetcherCredentials}";
  grpc_call_credentials* creds =
  ...
  // First request: should trigger email fetch and token fetch, but NOT RAB
  // fetch.
  auto state = RequestMetadataState::NewInstance(absl::OkStatus(), emd);
  // We reuse compute_engine_httpcli_get_success_with_rab_override but it will
  // fail if it sees a RAB request because we won't match the path if we don't
  // expect it (or we can use a stricter override). Actually, let's use a custom
  // override that fails on RAB fetch.
  HttpRequest::SetOverride(
      [](const grpc_http_request* request, const URI& uri, Timestamp deadline,
         grpc_closure* on_done, grpc_http_response* response) {
        if (uri.authority() == "metadata.google.internal.") {
          return compute_engine_httpcli_get_success_with_rab_override(
