...
class RegionalAccessBoundaryFetcherTest : public ::testing::Test {
 protected:
  bool has_cache() {
    MutexLock lock(&creds_->regional_access_boundary_fetcher_->cache_mu_);
    return creds_->regional_access_boundary_fetcher_->cache_.has_value();
  }
  std::string cached_encoded_locations() {
    MutexLock lock(&creds_->regional_access_boundary_fetcher_->cache_mu_);
    return creds_->regional_access_boundary_fetcher_->cache_->encoded_locations;
  }
  int cooldown_multiplier() {
    return creds_->regional_access_boundary_fetcher_->cooldown_multiplier_;
  }
  void set_cache(RegionalAccessBoundary cache) {
    MutexLock lock(&creds_->regional_access_boundary_fetcher_->cache_mu_);
    creds_->regional_access_boundary_fetcher_->cache_ = cache;
  }
  void set_cooldown_deadline(Timestamp t) {
    MutexLock lock(&creds_->regional_access_boundary_fetcher_->cache_mu_);
    creds_->regional_access_boundary_fetcher_->cooldown_deadline_ = t;
  }

  bool fetch_in_flight() {
    return creds_->regional_access_boundary_fetcher_->fetch_in_flight_;
  }
  void set_fetch_in_flight(bool val) {
    creds_->regional_access_boundary_fetcher_->fetch_in_flight_ = val;
  }
  bool has_retry_timer() {
    return creds_->regional_access_boundary_fetcher_->retry_timer_handle_
        .has_value();
  }
  Timestamp cooldown_deadline() {
    return creds_->regional_access_boundary_fetcher_->cooldown_deadline_;
  }
...
TEST_F(RegionalAccessBoundaryFetcherTest, CacheMissTriggersFetch) {
  ...
  auto result = creds_->regional_access_boundary_fetcher_->Fetch(
      "https://googleapis.com", "", std::move(metadata_));
...
TEST_F(RegionalAccessBoundaryFetcherTest, CacheHitDoesNotTriggerFetch) {
  set_cache(RegionalAccessBoundary{
      "us-west1", {"us-west1"}, Timestamp::Now() + Duration::Seconds(7200)});
  metadata_->Append("authorization", Slice::FromStaticString("Bearer token"),
                    [](absl::string_view, const Slice&) { abort(); });
...
TEST_F(RegionalAccessBoundaryFetcherTest, ExpiredCacheTriggersFetch) {
  set_cache(RegionalAccessBoundary{
      "us-west1", {"us-west1"}, Timestamp::Now() - Duration::Seconds(100)});
  metadata_->Append("authorization", Slice::FromStaticString("Bearer token"),
                    [](absl::string_view, const Slice&) { abort(); });
  auto result = creds_->regional_access_boundary_fetcher_->Fetch(
      "https://googleapis.com", "", std::move(metadata_));
...
TEST_F(RegionalAccessBoundaryFetcherTest, CooldownPreventsFetch) {
  set_cooldown_deadline(Timestamp::Now() + Duration::Seconds(100));
...
TEST_F(RegionalAccessBoundaryFetcherTest, InvalidUriParsing) {
  metadata_->Append("authorization", Slice::FromStaticString("Bearer token"),
                    [](absl::string_view, const Slice&) { abort(); });
  auto result = creds_->regional_access_boundary_fetcher_->Fetch(
      "invalid_uri_!@#$", "", std::move(metadata_));
  EXPECT_FALSE(fetch_in_flight());
}
...
TEST_F(RegionalAccessBoundaryFetcherTest, MalformedJsonResponse) {
  ...
  EXPECT_GT(cooldown_deadline(), Timestamp::Now());
}
...
TEST_F(RegionalAccessBoundaryFetcherTest, ValidJsonWithNonStringLocations) {
  ...
  EXPECT_TRUE(has_cache());
  EXPECT_EQ(cached_encoded_locations(), "us-west1");
  // The non-string location (123) should be dropped, meaning two valid
  // locations remain. We can't query the vector elements directly from the
  // test's view unless we add accessors. But verifying caching is successful is
...
TEST_F(RegionalAccessBoundaryFetcherTest, CooldownResetsOnSuccess) {
  ...
  metadata_1->Append("authorization", Slice::FromStaticString("Bearer token"),
                     [](absl::string_view, const Slice&) { abort(); });
  metadata_1->Set(HttpAuthorityMetadata(),
                  Slice::FromStaticString("googleapis.com"));
  auto result = creds_->regional_access_boundary_fetcher_->Fetch(
      "https://googleapis.com", "", std::move(metadata_1));
  ExecCtx::Get()->Flush();
  ...
  HttpRequest::SetOverride(httpcli_get_valid_json, nullptr, nullptr);
  exec_ctx.TestOnlySetNow(Timestamp::Now() + Duration::Minutes(16));
  ExecCtx::Get()->Flush();
  ...
  metadata_success->Set(HttpAuthorityMetadata(),
                        Slice::FromStaticString("googleapis.com"));
  result = creds_->regional_access_boundary_fetcher_->Fetch(
      "https://googleapis.com", "", std::move(metadata_success));
  ExecCtx::Get()->Flush();
  ...
  set_cache(RegionalAccessBoundary{"", {}, Timestamp::InfPast()});
  ...
  metadata_fail->Set(HttpAuthorityMetadata(),
                     Slice::FromStaticString("googleapis.com"));
  result = creds_->regional_access_boundary_fetcher_->Fetch(
      "https://googleapis.com", "", std::move(metadata_fail));
  ExecCtx::Get()->Flush();
...
TEST_F(RegionalAccessBoundaryFetcherTest, CacheSoftExpirationTriggersRefresh) {
  ...
  Timestamp soft_expired_timestamp =
      now +
      Duration::Seconds(
          GRPC_REGIONAL_ACCESS_BOUNDARY_SOFT_CACHE_GRACE_DURATION_SECS / 2);

  set_cache(
      RegionalAccessBoundary{"us-east1", {"us-east1"}, soft_expired_timestamp});
  ...
  auto result = creds_->regional_access_boundary_fetcher_->Fetch(
      "https://googleapis.com", "", std::move(metadata_soft_expired));
