...
class RegionalAccessBoundaryFetcherTest : public ::testing::Test {
 protected:
  ...
  int retry_count() {
    MutexLock lock(&creds_->regional_access_boundary_fetcher_->cache_mu_);
    return creds_->regional_access_boundary_fetcher_->num_retries_;
  }
  ...
  bool fetch_in_flight() {
    MutexLock lock(&creds_->regional_access_boundary_fetcher_->cache_mu_);
    return creds_->regional_access_boundary_fetcher_->pending_request_ !=
           nullptr;
  }
...
TEST_F(EmailFetcherTest, FetchesEmailAndThenRab) {
  ...
  grpc_polling_entity pollent =
      grpc_polling_entity_create_from_pollset(nullptr);
...
TEST_F(EmailFetcherTest, EmailFetchBackoffRespected) {
  ...
  exec_ctx.TestOnlySetNow(Timestamp::Now() + Duration::Seconds(2));
