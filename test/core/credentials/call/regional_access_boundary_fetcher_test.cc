...
TEST_F(RegionalAccessBoundaryFetcherTest, CacheMissTriggersFetch) {
  ...
  creds_->regional_access_boundary_fetcher_->Fetch("https://googleapis.com", "",
                                                   *metadata_);
...
TEST_F(RegionalAccessBoundaryFetcherTest, InvalidUriParsing) {
  metadata_->Append("authorization", Slice::FromStaticString("Bearer token"),
                    [](absl::string_view, const Slice&) { abort(); });
  creds_->regional_access_boundary_fetcher_->Fetch("invalid_uri_!@#$", "",
                                                   *metadata_);
  EXPECT_FALSE(fetch_in_flight());
}
...
TEST_F(RegionalAccessBoundaryFetcherTest, NonGoogleApisEndpointIgnored) {
  ...
  metadata_->Set(HttpAuthorityMetadata(),
                 Slice::FromStaticString("fake-googleapis.com"));
  creds_->regional_access_boundary_fetcher_->Fetch("https://googleapis.com", "",
                                                   *metadata_);
  EXPECT_FALSE(fetch_in_flight());
}
