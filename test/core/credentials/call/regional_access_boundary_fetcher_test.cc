...
TEST_F(RegionalAccessBoundaryFetcherTest, CacheMissTriggersFetch) {
  ...
  auto promise = creds_->regional_access_boundary_fetcher_->Fetch("https://googleapis.com", std::move(metadata_));
...
TEST_F(RegionalAccessBoundaryFetcherTest, InvalidUriParsing) {
  metadata_->Append("authorization", Slice::FromStaticString("Bearer token"), [](absl::string_view, const Slice&) { abort(); });
  auto promise = creds_->regional_access_boundary_fetcher_->Fetch("invalid_uri_!@#$", std::move(metadata_));
  EXPECT_FALSE(fetch_in_flight());
  Poll<absl::StatusOr<ClientMetadataHandle>> poll = promise();
  ASSERT_TRUE(poll.ready());
}
