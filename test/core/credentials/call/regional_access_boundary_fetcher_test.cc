...
class RegionalAccessBoundaryFetcherTest : public ::testing::Test {
 protected:
  ...
  int retry_count() {
    MutexLock lock(&fetcher_->cache_mu_);
    return fetcher_->num_retries_;
  }
  std::string cached_encoded_locations() {
    MutexLock lock(&fetcher_->cache_mu_);
    return fetcher_->cache_->encoded_locations;
  }
  int cooldown_multiplier() {
    MutexLock lock(&fetcher_->cache_mu_);
    return fetcher_->cooldown_multiplier_;
  }
  void set_cache(RegionalAccessBoundary cache) {
    MutexLock lock(&fetcher_->cache_mu_);
    fetcher_->cache_ = cache;
  }
  void set_cooldown_deadline(Timestamp t) {
    MutexLock lock(&fetcher_->cache_mu_);
    fetcher_->cooldown_deadline_ = t;
  }
  ...
  Timestamp cooldown_deadline() {
    MutexLock lock(&fetcher_->cache_mu_);
    return fetcher_->cooldown_deadline_;
  }
...
TEST_F(RegionalAccessBoundaryFetcherTest, RetryableHttpErrors) {
  ...
  Tick(Duration::Seconds(1) * 1.1 +
       Duration::Milliseconds(100));  // Initial backoff + jitter + buffer
  fetcher_->Fetch("", *metadata_);
  EXPECT_TRUE(fetch_in_flight());
  ExecCtx::Get()->Flush();
