...
#define GRPC_REGIONAL_ACCESS_BOUNDARY_SOFT_CACHE_GRACE_DURATION_SECS \
...
struct RegionalAccessBoundary {
  ...
  bool isValid() const { return expiration > grpc_core::Timestamp::Now(); }

  bool isSoftExpired() const {
    return (expiration -
            grpc_core::Duration::Seconds(
                GRPC_REGIONAL_ACCESS_BOUNDARY_SOFT_CACHE_GRACE_DURATION_SECS)) <
           grpc_core::Timestamp::Now();
  }
};
...
class RegionalAccessBoundaryFetcher
    : public RefCounted<RegionalAccessBoundaryFetcher> {
 public:
   ...
   * flight.
   */
  ClientMetadataHandle Fetch(std::string lookup_url, std::string access_token,
 ...
 private:
  ...
  Timestamp cooldown_deadline_ = Timestamp::ProcessEpoch();
  BackOff backoff_;
