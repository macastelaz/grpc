...
class RegionalAccessBoundaryFetcher final
 ...
 private:
  ...
  void OnFetchSuccess(std::string encoded_locations,
                      std::vector<std::string> locations);
...
class RegionalAccessBoundaryFetcher::Request final
 ...
 public:
  Request(WeakRefCountedPtr<RegionalAccessBoundaryFetcher> fetcher, URI uri,
          absl::string_view access_token);

  ~Request() override { grpc_http_response_destroy(&response_); }

  void Start();
  OrphanablePtr<Request> MakeRetryRequest();
...
class EmailFetcher : public DualRefCounted<EmailFetcher> {
 ...
 private:
  ...
  std::variant<std::monostate, OrphanablePtr<EmailRequest>,
