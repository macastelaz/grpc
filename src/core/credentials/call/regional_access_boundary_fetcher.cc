...
RegionalAccessBoundaryFetcher::Request::Request(
    WeakRefCountedPtr<RegionalAccessBoundaryFetcher> fetcher, URI uri,
...
void RegionalAccessBoundaryFetcher::Request::Start() {
  ...
  http_request_ = HttpRequest::Get(uri_,
...
OrphanablePtr<RegionalAccessBoundaryFetcher::Request>
RegionalAccessBoundaryFetcher::Request::MakeRetryRequest() {
...
void RegionalAccessBoundaryFetcher::Request::OnResponse(
  ...
  if (fetcher != nullptr) {
    if (success) {
      fetcher->OnFetchSuccess(std::move(encoded_locations),
                              std::move(locations));
    } else {
      fetcher->OnFetchFailure(
...
void RegionalAccessBoundaryFetcher::OnFetchSuccess(
    std::string encoded_locations, std::vector<std::string> locations) {
...
void RegionalAccessBoundaryFetcher::Fetch(absl::string_view access_token,
  ...
  }
  {
    ...
    if ((!cache_.has_value() ||
      ...
      pending_request_ =
          MakeOrphanable<Request>(WeakRef(), *lookup_uri_, access_token);
      pending_request_->Start();
    }
    // If we have a cached non-expired token, use it.
    if (cache_.has_value() && cache_->expiration > now) {
      std::cout << "Using cached regional access boundary: "
                << cache_->encoded_locations << std::endl;
  ...
  void Start() {
    ...
    if (!uri.ok()) {
      LOG_EVERY_N_SEC(ERROR, 60)
          << "Failed to create URI for service account email fetch: "
          << uri.status();
      return;
    }
    GRPC_CLOSURE_INIT(&closure_, OnResponseWrapper, this,
...
void EmailFetcher::StartEmailFetch(grpc_polling_entity* pollent) {
  ...
  // We keep a temporary ref to start it, but transfer ownership to state_
  // first.
...
void EmailFetcher::OnEmailFetchComplete(absl::string_view email) {
  ...
  if (std::holds_alternative<OrphanablePtr<EmailRequest>>(state_)) {
    std::string rab_url = absl::StrFormat(
        ...
        email);
    state_ =
        MakeRefCounted<RegionalAccessBoundaryFetcher>(rab_url, event_engine_);
    backoff_.Reset();
  }
}
