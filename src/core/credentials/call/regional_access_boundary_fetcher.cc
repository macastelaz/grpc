...
constexpr Duration kRegionalAccessBoundaryBaseCooldownDuration =
    Duration::Minutes(15);
constexpr Duration kRegionalAccessBoundaryMaxCooldownDuration =
    Duration::Hours(1);
...
RegionalAccessBoundaryFetcher::RegionalAccessBoundaryRequest::
    RegionalAccessBoundaryRequest(URI uri, absl::string_view access_token)
...
void RegionalAccessBoundaryFetcher::StartRegionalAccessBoundaryFetch(
  ...
  req->http_request_ = HttpRequest::Get(
      req->uri_,
      nullptr,  // channel_args
      &req->pollent_, &request, Timestamp::Now() + Duration::Seconds(60),
      &req->closure_, &req->response_,
...
void RegionalAccessBoundaryFetcher::OnRegionalAccessBoundaryResponse(
  ...
  if (success) {
    ...
    if (!success) {
      ...
      if (cooldown_multiplier_ * kRegionalAccessBoundaryBaseCooldownDuration <
...
void RegionalAccessBoundaryFetcher::RetryFetchRegionalAccessBoundary(
  ...
  if (error.ok()) {
    RefCountedPtr<RegionalAccessBoundaryRequest> new_req =
        MakeRefCounted<RegionalAccessBoundaryRequest>(raw_req->uri_,
                                                      raw_req->access_token_);
...
void RegionalAccessBoundaryFetcher::Fetch(absl::string_view access_token,
  ...
  if (lookup_url_.empty()) {
    ...
    // information to construct the URL, such as a missing workforce/workload
    // pool ID or service account email.
    return;
  }
  absl::StatusOr<URI> request_uri = URI::Parse(lookup_url_);
  if (!request_uri.ok()) {
  ...
  }
  {
    ...
    if (has_valid_cache) {
      initial_metadata.Append(
          ...
          Slice::FromCopiedString(cache_->encoded_locations),
          [](absl::string_view, const Slice&) { abort(); });
    }
    bool should_skip_new_lookup = (has_valid_cache && !cache_is_soft_expired) ||
                                  pending_request_ != nullptr ||
                                  Timestamp::Now() < cooldown_deadline_;
    if (should_skip_new_lookup) {
      return;
    }
  }
  RefCountedPtr<RegionalAccessBoundaryRequest> req =
      MakeRefCounted<RegionalAccessBoundaryRequest>(std::move(*request_uri),
                                                    access_token);
  // Do not wait for the regional access boundary to be fetched as we do not
  // want to block the underlying call.
