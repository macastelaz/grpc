...
struct RegionalAccessBoundaryFetcher::RegionalAccessBoundaryRequest
  ...
  int num_retries = 0;
  RefCountedPtr<RegionalAccessBoundaryFetcher> fetcher;
...
void RegionalAccessBoundaryFetcher::StartRegionalAccessBoundaryFetch(
  ...
  grpc_closure* closure =
      GRPC_CLOSURE_CREATE(OnRegionalAccessBoundaryResponseWrapper, req.get(),
                          grpc_schedule_on_exec_ctx);

  req->http_request = HttpRequest::Get(req->uri,
                                       nullptr,  // channel_args
                                       &req->pollent, &request,
                                       Timestamp::Now() + Duration::Seconds(60),
                                       closure, &req->response,
...
void RegionalAccessBoundaryFetcher::OnRegionalAccessBoundaryResponse(
  ...
  if (should_retry) {
    ...
    req->num_retries++;
    Duration delay = backoff_.NextAttemptDelay();
...
ClientMetadataHandle RegionalAccessBoundaryFetcher::Fetch(
    std::string lookup_url, ClientMetadataHandle initial_metadata) {
  ...
  if (absl::StrContains(authority, kRegionalEndpoint) ||
      !absl::StrContains(authority, kGoogleApisEndpoint)) {
    return initial_metadata;
  }

  {
    ...
    auto shouldSkipNewLookup = hasValidCache || fetch_in_flight_ ||
  ...
  if (auth_val.has_value()) {
    ...
    LOG(WARNING)
        << "No access token was found in the metadata for this credential "
        << "and therefore the lookup would fail. A lookup will not be "
           "attempted.";
    return initial_metadata;
  }
  ...
  // want to block the underlying call. Instead, immediately return the initial
  // metadata. The lookup will happen in the background and the result will be
