...
void RegionalAccessBoundaryFetcher::Fetch(absl::string_view lookup_url,
  ...
  absl::StatusOr<URI> request_uri = URI::Parse(lookup_url);
  if (!request_uri.ok()) {
  ...
  }
  {
    ...
    bool should_skip_new_lookup =
        (has_valid_cache && !should_trigger_async_fetch) || fetch_in_flight_ ||
