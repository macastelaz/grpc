...
void RegionalAccessBoundaryFetcher::OnRegionalAccessBoundaryResponse(
  ...
  if (error.ok() && req->response.status == 200) {
    auto json = JsonParse(
        absl::string_view(req->response.body, req->response.body_length));
  ...
  if (!success && !absl::IsCancelled(error) &&
      req->num_retries < kMaxRegionalAccessBoundaryRetries) {
...
ClientMetadataHandle RegionalAccessBoundaryFetcher::Fetch(
    std::string lookup_url, std::string access_token,
...

  {
    ...
    auto shouldSkipNewLookup = (hasValidCache && !shouldTriggerAsyncFetch) ||
                               fetch_in_flight_ ||
