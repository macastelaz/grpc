...
grpc_service_account_jwt_access_credentials::GetRequestMetadata(
  ...
  return grpc_core::Immediate(regional_access_boundary_fetcher_->Fetch(build_regional_access_boundary_url(), std::move(initial_metadata)));
...
grpc_service_account_jwt_access_credentials::
    ...
    : key_(key), regional_access_boundary_fetcher_(grpc_core::MakeRefCounted<grpc_core::RegionalAccessBoundaryFetcher>()) {
