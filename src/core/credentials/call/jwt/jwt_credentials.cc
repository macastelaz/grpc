...
grpc_service_account_jwt_access_credentials_create_from_auth_json_key(
  ...
  return grpc_core::MakeRefCounted<grpc_service_account_jwt_access_credentials>(
      key, token_lifetime, encoded_locations);
}
...
grpc_call_credentials* grpc_service_account_jwt_access_credentials_create(
  ...
  return grpc_service_account_jwt_access_credentials_create_with_encoded_locations(
...
grpc_service_account_jwt_access_credentials_create_with_encoded_locations(
  ...
  if (GRPC_TRACE_FLAG_ENABLED(api)) {
    ...
    VLOG(2) << "grpc_service_account_jwt_access_credentials_create_ex("
            << "json_key=" << clean_json
            ...
            << " }, encoded_locations="
            << (encoded_locations == nullptr ? "NULL" : encoded_locations)
            << ", reserved=" << reserved << ")";
    gpr_free(clean_json);
  }
